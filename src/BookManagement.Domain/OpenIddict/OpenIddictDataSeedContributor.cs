...
namespace BookManagement.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("BookManagement") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "BookManagement", DisplayName = "BookManagement API", Resources = { "BookManagement" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        if (!webClientId.IsNullOrWhiteSpace())
        {
            ...
            /* BookManagement_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["BookManagement_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["BookManagement_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["BookManagement_Swagger:RootUrl"]?.TrimEnd('/');
